...
class IMDB(object):
    ...
    def gt_imdb(self):
        ...
        with open(cache_file, 'wb') as f:
            pickle.dump(gt_imdb, f, pickle.HIGHEST_PROTOCOL)
        return gt_imdb
    ...
    def image_path_from_index(self, index):
        ...
        image_file: str
            full path of image
        """
        # 这个就是从截取好的12×12的文件夹中拿到图片 作为训练集送入到训练网络中
        # if '/Users/qiuxiaocong/Downloads/WIDER_train' in index:
        #     assert os.path.exists(index), 'Path does not exist: {}'.format(index)
        #     return index

        # image_file = os.path.join(self.data_path, 'images', index)
        image_file = os.path.join(self.data_path, 'images/originalPics', index)
    ...
    def load_annotations(self):
        """Load annotations
        这个应该就是加载训练集，这里面包含12×12训练图片的路径/类别/以及框回归的参数
        Parameters:
        ----------
        Returns:
        -------
        ...
        for i in range(self.num_images):
            annotation = annotations[i].strip()
            index = annotation
...
#                imdb_['gt_boxes'] = boxes
    ...
    def append_flipped_images(self, imdb):
        ...
        for i in range(len(imdb)):
            ...
            imdb.append(entry)
        self.image_set_index *= 2  # 每张图片都做水平反转,故数据量会翻倍
        return imdb
